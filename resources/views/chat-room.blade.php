<x-filament-panels::page>
    <div class="flex h-[75vh] bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm relative"
        wire:poll.3s>

        <div
            class="{{ $showChatDetail ? 'hidden' : 'flex' }} md:flex w-full md:w-80 border-r border-gray-200 dark:border-gray-700 flex-col bg-gray-50/30 dark:bg-gray-900/50">
            ...
            <div class="overflow-y-auto flex-1">
                @foreach ($this->getLeads() as $lead)
                    <div wire:click="selectLead({{ $lead->id }})"
                        ...
                        <div
                            class="w-10 h-10 rounded-full bg-primary-500 flex-shrink-0 flex items-center justify-center text-white font-bold">
        ...
        <div class="{{ $showChatDetail ? 'flex' : 'hidden' }} md:flex flex-1 flex-col bg-white dark:bg-gray-950">
            @if ($activeLeadId)
                <div
                    class="p-4 border-b dark:border-gray-700 flex items-center gap-3 bg-white dark:bg-gray-900 shadow-sm">
                    ...
                    <div
                        class="w-8 h-8 rounded-full bg-gray-200 dark:bg-gray-700 flex-shrink-0 flex items-center justify-center text-xs font-bold text-primary-600">
                ...
                <div class="flex-1 p-4 md:p-6 overflow-y-auto flex flex-col gap-4 bg-[#f0f2f5] dark:bg-gray-950">
                    @foreach ($this->getMessages() as $msg)
                        <div class="flex {{ $msg->direction == 'outbound' ? 'justify-end' : 'justify-start' }}">
                            <div
                                class="max-w-[85%] md:max-w-[70%] px-4 py-2 rounded-2xl shadow-sm text-sm
